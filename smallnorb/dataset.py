...
class SmallNORBDataset:
	...
	def export_to_jpg_2(self, export_dir):
		...
		if self.initialized:
			...
			dir_lt_path = join(export_dir, 'lt', 'all')
			makedirs(dir_lt_path)
			dir_rt_path = join(export_dir, 'rt', 'all')
			makedirs(dir_rt_path)
			'''
			for split_name in ['train', 'test']:
				...
				for _, norb_example in enumerate(tqdm(self.data[split_name])):
					...
					image_lt_path = join(dir_lt_path, 'lt_{:01d}_{:01d}_{:02d}_{:01d}_{:01d}.png'.format(category, instance, azimuth, elevation, lighting))
					image_rt_path = join(dir_rt_path, 'rt_{:01d}_{:01d}_{:02d}_{:01d}_{:01d}.png'.format(category, instance, azimuth, elevation, lighting))
